package org.apache.spark.util.collection.nvm
...
private[spark] class nvmExternalSorter[K, V, C](
    context: TaskContext,
  ...
  logDebug("Initialize nvmExternalSorter")
  private[this] val pmemBlockOutputStreamArray: ArrayBuffer[nvmBlockOutputStream] = ArrayBuffer[nvmBlockOutputStream]()
  ...
  def setPartitionByteBufferArray(writerArray: Array[nvmBlockOutputStream] = null): Unit = {
  ...
  def getPartitionByteBufferArray(stageId: Int, partitionId: Int): nvmBlockOutputStream = {
    if (mapStage) {
      pmemBlockOutputStreamArray(partitionId)
    } else {
      pmemBlockOutputStreamArray += new nvmBlockOutputStream(
        context.taskMetrics(),
  ...
  // 需要聚合时调用，强制调用spill->spillMemoryIteratorToPmem，遍历collection将collection的数据写入NVM
  ...
  override protected[this] def maybeSpill(collection: WritablePartitionedPairCollection[K, C], currentMemory: Long): Boolean = {
    ...
    // 放入数据每32次且currentMemory(estimatedSize) 大于 inMemoryCollectionSizeThreshold(=5M)
    if (elementsRead % 32 == 0 && currentMemory >= nvmconf.inMemoryCollectionSizeThreshold) {
      shouldSpill = currentMemory >= nvmconf.inMemoryCollectionSizeThreshold
      logDebug("maybeSpill: shouldSpill=" + shouldSpill + ", currentMemory=" + currentMemory + ", inMemoryCollectionSizeThreshold=" + nvmconf.inMemoryCollectionSizeThreshold)
    }
    if (shouldSpill) {
      spill(collection)
    }
  ...
  private[this] def spillMemoryIteratorToPmem(inMemoryIterator: WritablePartitionedIterator): Unit = {
    var buffer: nvmBlockOutputStream = null
    var cur_partitionId = -1
    while (inMemoryIterator.hasNext) {
      val partitionId = inMemoryIterator.nextPartition()
      if (cur_partitionId != partitionId) {
        if (cur_partitionId != -1) {      // 保证已经writeNext将数据写入ByteBuffer
          buffer.maybeSpill(true)         // 强制flush到NVM，底层是将byteBuffer刷入NVM
        }
      ...
      // writeNext调用destructiveSortedWritablePartitionedIterator.writeNext
      // 调用 nvmBlockOutputStream.write 将数据写入PmemOutputStream.ByteBuffer，然后移到下一个
      // 最后调用 persistentMemoryWriter.setPartition将数据块写入
  ...
  class SpillReader(pmemBlockOutputStream: nvmBlockOutputStream) {
    // Each spill reader is relate to one partition
    // which is different from spark original codes (relate to one spill file)
    val pmemBlockInputStream = new nvmBlockInputStream[K, C](pmemBlockOutputStream, serializer)
    var nextItem: (K, C) = _
