...
class nvmManagedBuffer(pmHandler: PersistentMemoryHandler, blockId: String) extends ManagedBuffer with Logging {
  ...
  override def createInputStream(): InputStream = {
    logDebug("nvmManagedBuffer DEBUG: createInputStream, blockId=" + blockId)
    if (inputStream == null) {
      inputStream = new nvmInputStream(pmHandler, blockId)
    }
    inputStream
  }
  ...
  override def convertToNetty(): Object = {
    val data_length = size().toInt
    val in = createInputStream()
    Unpooled.wrappedBuffer(in.asInstanceOf[nvmInputStream].getByteBufferDirectAddr, data_length, false)
  }
}
